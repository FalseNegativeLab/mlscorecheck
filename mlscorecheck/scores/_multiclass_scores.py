...
def multiclass_score_macro(confusion_matrix: np.ndarray, 
    ...
    Args:
        confusion_matrix (np.ndarray): the confusion matrix (true labels in rows)
...
def multiclass_score_micro(confusion_matrix: np.ndarray, 
    ...
    for idx, count in enumerate(counts):
        ...
        params["tn"] += (
            np.sum(confusion_matrix[0:idx, 0:idx])
            + np.sum(confusion_matrix[idx + 1 :, idx + 1 :])
            + np.sum(confusion_matrix[:idx, idx + 1 :])
            + np.sum(confusion_matrix[idx + 1 :, :idx])
        )
    ...
    return safe_call(
        score_function, params | additional_params, score_specifications[name].get("nans")
    )
...
def multiclass_score(confusion_matrix: np.ndarray, 
    ...
    """
    if average == "micro":
        return multiclass_score_micro(confusion_matrix, score_function, additional_params, name)
    if average == "macro":
        return multiclass_score_macro(confusion_matrix, score_function, additional_params, name)
    if average == "weighted":
        return multiclass_score_weighted(confusion_matrix, score_function, additional_params, name)
...
def multiclass_positive_predictive_value(*, confusion_matrix: np.ndarray, average: str) -> float:
    ...
    Returns:
        float: the score
    """
    return multiclass_score(
        confusion_matrix, positive_predictive_value_standardized, average, name="ppv"
    )
...
def multiclass_negative_predictive_value(*, confusion_matrix: np.ndarray, average: str) -> float:
    ...
    """
    return multiclass_score(
        confusion_matrix, negative_predictive_value_standardized, average, name="npv"
    )
...
def multiclass_unified_performance_measure(*, confusion_matrix: np.ndarray, average: str) -> float:
    ...
    """
    return multiclass_score(
        confusion_matrix, unified_performance_measure_standardized, average, name="upm"
    )
...
def multiclass_fowlkes_mallows_index(*, confusion_matrix: np.ndarray, average: str) -> float:
    ...
    """
    return multiclass_score(
        confusion_matrix, fowlkes_mallows_index_standardized, average, name="fm"
    )
...
def multiclass_positive_likelihood_ratio(*, confusion_matrix: np.ndarray, average: str) -> float:
    ...
    """
    return multiclass_score(
        confusion_matrix, positive_likelihood_ratio_standardized, average, name="lrp"
    )
...
def multiclass_negative_likelihood_ratio(*, confusion_matrix: np.ndarray, average: str) -> float:
    ...
    """
    return multiclass_score(
        confusion_matrix, negative_likelihood_ratio_standardized, average, name="lrn"
    )
...
def multiclass_matthews_correlation_coefficient(*, confusion_matrix: np.ndarray, average: str) -> float:
    ...
    """
    return multiclass_score(
        confusion_matrix, matthews_correlation_coefficient_standardized, average, name="mcc"
    )
...
def multiclass_bookmaker_informedness(*, confusion_matrix: np.ndarray, average: str) -> float:
    ...
    """
    return multiclass_score(
        confusion_matrix, bookmaker_informedness_standardized, average, name="bm"
    )
...
def multiclass_diagnostic_odds_ratio(*, confusion_matrix: np.ndarray, average: str) -> float:
    ...
    """
    return multiclass_score(
        confusion_matrix, diagnostic_odds_ratio_standardized, average, name="dor"
    )
...
def multiclass_cohens_kappa(*, confusion_matrix: np.ndarray, average: str) -> float:
    ...
    Returns:
        float: the score
    """
    return multiclass_score(confusion_matrix, cohens_kappa_standardized, average, name="kappa")
