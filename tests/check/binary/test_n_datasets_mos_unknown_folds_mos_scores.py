...
def test_successful(random_seed: int, subset: list):
    ...
    results = check_n_datasets_mos_unknown_folds_mos(evaluations=experiment['evaluations'],
                                                            scores=scores,
                                                            eps=1e-4)
...
def test_exception():
    ...
    with pytest.raises(ValueError):
        check_n_datasets_mos_unknown_folds_mos(evaluations=[{'aggregation': 'som'}],
                                                        scores={},
                                                        eps=1e-4)

    with pytest.raises(ValueError):
        check_n_datasets_mos_unknown_folds_mos(evaluations=[{'aggregation': 'mos',
                                                                    'fold_score_bounds': {}}],
                                                        scores={},
                                                        eps=1e-4)
