...
def test_consistency(random_seed: int, rounding_decimals: int):
    ...
    result = check_n_datasets_mos_known_folds_mos(evaluations=experiment['evaluations'],
                                                    scores=scores,
...
def test_exception():
    ...
    with pytest.raises(ValueError):
        check_n_datasets_mos_known_folds_mos(evaluations=[{'aggregation': 'som'}],
                                                scores={},
                                                eps=1e-4)

    with pytest.raises(ValueError):
        check_n_datasets_mos_known_folds_mos(evaluations=[{'aggregation': 'mos',
                                                                    'fold_score_bounds': {}}],
                                                scores={},
                                                eps=1e-4)
